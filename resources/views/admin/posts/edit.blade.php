...
@section('title')
<h1 class="header-panel">Edit Post: <b>{{$post->title}}</b> </h1>
@endsection
@section('content')
   ...
   <div class="row" >
      <div class="col-sm-8" >
            ...
            <div class="form-group">
              <input required="required" value="{{$post->title}}" placeholder="Enter title here" type="text" name="title" class="form-control" />
            </div>
            <div class="form-group">
            ...
            <div id="seo" class="postbox">
                ...
                <div class="collapse" id="seo-content" >
                    <div class="form-group">
                        <input value="{{$post->seo->title or ''}}" placeholder="Title" type="text" name="seo_title" class="form-control" />
                    </div>
                    <div class="form-group">
                        <textarea name='seo_description' class="form-control">{{$post->seo->description or ""}}</textarea>
                    </div>
                     <div class="form-group">
                        <input value="{{$post->seo->share_image or ''}}" placeholder="Enter image share" type="text" name="share_image" class="form-control" />
                    </div>
                     <div class="form-group">
                        <input value="{{$post->seo->share_url or ''}}" placeholder="Get info from this url" type="text" name="share_url" class="form-control" />
                    </div>
                </div>
          </div>
      </div>
      <div class="col-sm-4">
              ...
              <div class="row">
                  ...
                  <ul id="categorychecklist">
                      @foreach($categories as $cat)
                        ...
                        <input type="checkbox" @if(in_array($cat->name, array_column($post->categories->toArray(),'name'))) checked @endif name="categories[]" value="{{$cat->id}}" />
                           {{$cat->name}}
                        </li>
                      @endforeach
                  </ul>
          ...
          <div id="tag" class="postbox">
              ...
              <div class="row">
                  <div class="col-md-12 form-inline">
                            ...
                            <div id="show-tags" class="col-12">
                              @foreach($post->tags as $tag)
                              <span class="input-tag">
                                <i class="fa fa-times-circle btn-remove"></i>{{$tag->name}}
          ...
          <div id="thumbnail" class="postbox" >
                ...
                <div class="row">
                    <div class="col-12" >
                        ...
                        <div class="image-upload">
                             <img alt="Click to change thumbnail" src="{{$post->thumbnail}}" />
                ...
                <div class="row">
                    <div class="col-12">
                      <a href="" data-toggle="modal" data-target="#modal-upload">Edit featured image</a>
                    </div>
                </div>
          </div>
      </div>
...
<div  id="modal-upload" class="modal fade" tabindex="-1" role="dialog">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
         <button ng-click="updateThumbnail(root_url+info.url)" type="button" data-dismiss="modal" ng-disabled="!info" class="btn btn-primary">Set seautured image</button>
     </div>
    </div>
  </div>
</div>
